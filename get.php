<?php
	...
	class Get extends CI_Model {
		...
		public function get_progres($idTema)
		{
			$sql = "SELECT * FROM progres WHERE id_tema = " . $idTema;
			$result = $this->db->query($sql);

			if ($result->num_rows() > 0)
			{
				$prog = $result->result_array();
		...
		public function get_etapa($idEtapa)
		{
			$sql = "SELECT nume FROM etape WHERE id = " . $idEtapa;
			$result = $this->db->query($sql);

			if ($result->num_rows() > 0)
			{
				$etape = $result->result_array();
				$etapa = $etape[0];
